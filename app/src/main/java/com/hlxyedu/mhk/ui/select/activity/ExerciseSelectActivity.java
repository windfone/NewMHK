...
public class ExerciseSelectActivity extends RootActivity<ExerciseSelectPresenter> implements ExerciseSelectContract.View, XBaseTopBarImp {
     ...
     * @return
     */
    public static Intent newInstance(Context context, String examType) {
    ...
    protected void initEventAndData() {
        ...
        if (StringUtils.isEmpty(examType)) {
            questionType = "1";
            complexCb.performClick();
        } else {
            switch (examType) {
                case "TL":
                    listeningCb.performClick();
                    break;
                case "KY":
                    oralCb.performClick();
                    break;
                case "YD":
                    readingCb.performClick();
                    break;
                case "SM":
                    writeCb.performClick();
                    break;
                case "ZW":
                    compositionCb.performClick();
                    break;
            }
        }
    }
